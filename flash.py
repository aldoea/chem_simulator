...
def start_flash_isotermico(tf, p, zfDicc):
    ...
    for element in zfDicc:
        ...
        Zif = float(zfDicc[element]['value'].get())
    ...
    while True:
        psi_abs = abs((psi_kplus1 - psi) / psi)
        print('-- psi_abs:', psi_abs)
        if psi_abs < 0.001:
            print('º- psi_abs pass the error prove, Psi:', psi, 'psi_kplus1:', psi_kplus1)
            for element in zfDicc:
                ...
                if not zfDicc[element]['is_normalized']:
                    ...
                    if norm_xi < 0.001 and norm_yi < 0.001:
                        ...
                        for element in zfDicc:
                            zfDicc[element]['is_normalized'] = False
                        print('-> Recalculating psi_kplus1; psi:', psi, 'psi_kplus1:', psi_kplus1)
                        psi = psi_kplus1
                        psi_kplus1 = recalculate_psi(zfDicc, psi)
                        print('-> psi:', psi, 'psi_kplus1 now:', psi_kplus1)
                        normalized_counter = 0
                        break
        else:
            print('-> Recalculating psi_kplus1; psi:', psi, 'psi_kplus1:', psi_kplus1)
...
def calculate_psi_kplus1(zfDicc, psi):
    psi_kplus1 = 0.0
    for element in zfDicc:
        db_values = db.getElementValues(zfDicc[element]['db_row'])
        Zif = float(zfDicc[element]['value'].get())
        Kils = zfDicc[element]['Kils']
        tmp = (Zif * (1 - Kils)) / (1 + (psi * (Kils - 1)))
        psi_kplus1 += tmp
    return psi_kplus1
